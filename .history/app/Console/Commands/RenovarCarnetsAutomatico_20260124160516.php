    ...
    public function handle()
    {
        ...
        foreach ($carnetsProximosVencer as $carnetViejo) {
            try {
                ...
                $this->line("✅ Renovado: {$usuario->nombreCompleto} (Cédula: {$usuario->cedula})");
                $this->line("   Vencía: {$carnetViejo->fecha_vencimiento->format('d/m/Y')}");
                $this->line("   Nuevo vencimiento: {$nuevoCarnet->fecha_vencimiento->format('d/m/Y')}");
                $this->newLine();
