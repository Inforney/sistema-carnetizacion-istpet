...
class CarnetController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $codigoQr = 'ISTPET-' . date('Y') . '-' . $usuario->cedula;
        ...
        return redirect()->route('admin.carnets.index')->with(
            'success',
            'Carnet generado exitosamente para ' . $usuario->nombres . ' ' . $usuario->apellidos
        );
    }
    ...
    public function generarMasivo()
    {
        ...
        foreach ($estudiantesSinCarnet as $estudiante) {
            $codigoQr = 'ISTPET-' . date('Y') . '-' . $estudiante->cedula;
    ...
    /**
     * Descargar todos los carnets en PDF (NUEVO DISEÑO)
     */
    public function descargarMasivo()
    {
        try {
            ...
            // USAR VISTA NUEVA: pdf-masivo-nuevo.blade.php
            $pdf = Pdf::loadView('carnets.pdf-masivo-nuevo', compact('carnetes'))
    ...
    /**
     * Descargar carnet individual (NUEVO DISEÑO)
     */
    public function descargar($id)
    {
        try {
            ...
            return $pdf->download('carnet_' . $usuario->cedula . '_' . date('Y-m-d') . '.pdf');
        } catch (\Exception $e) {
            return back()->with('error', 'Error al generar el PDF: ' . $e->getMessage());
        }
    }
    ...
    /**
     * Descargar carnet individual con diseño viejo (BACKUP)
     */
    public function descargarViejo($id)
    {
        try {
            $carnet = Carnet::with('usuario')->findOrFail($id);
            ...
            return $pdf->download('carnet_viejo_' . $carnet->usuario->cedula . '.pdf');
        } catch (\Exception $e) {
            return back()->with('error', 'Error al generar el PDF: ' . $e->getMessage());
        }
    }
